function expConditions = locFfa_tmp()
...
try
  ...
  % scramble images: TODO: if or case
  numTiles = 10; % TODO: temporally set to 10
  nImages = length(Images);
  for iImage = 1:nImages
      Images(nImages+iImage).data = imscramble(Images(iImage).data, numTiles);
      Images(nImages+iImage).name = ['scr_' Images(iImage).name];
  end
...
HideCursor;
  ...
  [LBlank.VBLTimestamp, LBlank.StimulusOnsetTime, LBlank.FlipTimestamp, LBlank.Missed, LBlank.Beampos] ...
      = Screen('Flip', windowPtr, FBlank.VBLTimestamp + whenSecs4blank(nStim) + LBlankSecs);
  EndTime = now;
