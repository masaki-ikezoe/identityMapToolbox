...
% Blankは刺激として扱われません（names, durations, onsetsには入りません）。
...
Nstim = size(imgFileListCell, 2);
names = imgFileListCell(1, 1:Nstim-1); %The last stim, i.e. "Blank", is excluded.
...
onsets = cell(1, Nstim-1); %The last stim, i.e. "Blank", is excluded.
for k = 1:Nstim-1 %The last stim, i.e. "Blank", is excluded.
...
durations = cell(1, Nstim-1); %The last stim, i.e. "Blank", is excluded.
for k = 1:Nstim-1 %The last stim, i.e. "Blank", is excluded.
...
% output file
output = strrep(logFile, '.mat', 'info02.mat');
